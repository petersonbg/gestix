...
O sistema inclui automaticamente `HOSTNAME` e `HOSTNAME.local`, e o launcher
inclui a porta e o IP atual. Nao e necessario configurar IP fixo ou reserva no
roteador. Nunca exponha esse servidor diretamente na internet sem HTTPS, proxy
reverso e revisao de seguranca.
...
- anuncia `gestix.local` via mDNS e responde ao fallback UDP;
...
Os computadores clientes precisam apenas de um navegador. Nenhum launcher ou
outro programa precisa ser instalado neles.
