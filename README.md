...
A tela **Administração → Logs de Atividade** (`/administracao/logs/`) registra ações
relevantes como login, logout, criações, edições, cancelamentos, finalizações,
impressões, movimentações de estoque, operações de caixa, pagamentos, recebimentos,
backup e restauração. Administradores e gerentes podem consultar e filtrar por usuário,
ação, módulo, período e descrição.

Cada registro preserva data/hora, usuário, IP, user agent e, quando disponível, tipo e
identificador do objeto afetado. A auditoria é complementar: falhas ao gravar um log
não interrompem a operação principal. Logs não podem ser editados e somente um
superusuário pode excluí-los pelo Django Admin. Descrições de auditoria não devem
conter senhas, tokens, dados de cartão ou outros segredos.

### Backup e restauração

Administradores acessam **Administração → Backup e Restauração** em
`/administracao/backup/`. A tela permite gerar backups PostgreSQL no formato custom,
baixar arquivos protegidos e restaurar arquivos `.dump` ou `.backup` de até 500 MB.
Os arquivos gerados seguem o padrão `gestix_backup_YYYYMMDD_HHMMSS.dump`.
Os arquivos ficam em `BACKUP_ROOT` (por padrão, `backups/`), fora de `static/` e
`media/`, e nunca possuem URL pública.

Também é possível executar pelo terminal:

```bash
python manage.py gerar_backup
python manage.py restaurar_backup /caminho/arquivo.dump --confirmar
```

No Docker, `RUNNING_IN_DOCKER=True` identifica a execução no container web; ele inclui `pg_dump` e `pg_restore`, conecta-se ao serviço
PostgreSQL `db` e grava os arquivos no volume persistente `backups_data`. Para um
backup manual equivalente:

```bash
mkdir -p backups
docker compose exec -T db pg_dump -U gestix -d gestix -Fc > backups/gestix_backup_manual.dump
```

Antes de cada restauração, o sistema valida o arquivo com `pg_restore --list`, registra tentativas inválidas como erro no histórico e gera automaticamente um backup de segurança antes de aplicar qualquer dado.
Cada geração, download, restauração concluída ou falha também é registrada em **Logs de Atividade** para auditoria.
A restauração substitui objetos e dados atuais: faça-a em janela de manutenção,
confirme que nenhum usuário está operando o sistema e teste previamente a recuperação
em um banco descartável. Em instalações de maior porte, prefira o management command
ou uma fila de tarefas em vez de manter uma requisição web longa aberta.
