...
class SessaoUnicaUsuarioTests(TestCase):
    def setUp(self):
        User = get_user_model()
        self.usuario = User.objects.create_user(username='sessao-unica', password='senha')
        PerfilUsuario.objects.create(
            usuario=self.usuario,
            perfil=PerfilUsuario.Perfil.ADMINISTRADOR,
        )
        self.outro_usuario = User.objects.create_user(username='outro-usuario', password='senha')
        PerfilUsuario.objects.create(
            usuario=self.outro_usuario,
            perfil=PerfilUsuario.Perfil.VENDEDOR,
        )

    def login(self, client, username='sessao-unica', user_agent='Navegador Teste', ip='10.0.0.10'):
        return client.post(
            reverse('login'),
    ...
    def test_primeiro_login_cria_sessao_ativa(self):
        client = Client()

        response = self.login(client)

        self.assertEqual(response.status_code, 302)
        sessao = SessaoUsuario.objects.get(usuario=self.usuario, ativa=True)
        self.assertEqual(sessao.session_key, client.session.session_key)
        self.assertEqual(sessao.ip_usuario, '10.0.0.10')
        self.assertEqual(sessao.user_agent, 'Navegador Teste')
        self.assertIsNotNone(sessao.data_login)
        self.assertIsNotNone(sessao.ultimo_acesso)

    def test_segundo_login_encerra_sessao_anterior(self):
        primeiro = Client()
        segundo = Client()
        self.login(primeiro, user_agent='Primeiro navegador')
        chave_antiga = primeiro.session.session_key

        self.login(segundo, user_agent='Segundo navegador', ip='10.0.0.20')

        sessao_antiga = SessaoUsuario.objects.get(session_key=chave_antiga)
        sessao_nova = SessaoUsuario.objects.get(usuario=self.usuario, ativa=True)
        self.assertFalse(sessao_antiga.ativa)
        self.assertEqual(sessao_nova.session_key, segundo.session.session_key)
        self.assertEqual(SessaoUsuario.objects.filter(usuario=self.usuario, ativa=True).count(), 1)

    def test_sessao_antiga_e_removida_da_tabela_django_session(self):
        primeiro = Client()
        segundo = Client()
        self.login(primeiro)
        chave_antiga = primeiro.session.session_key
        self.assertTrue(Session.objects.filter(session_key=chave_antiga).exists())

        self.login(segundo)

        self.assertFalse(Session.objects.filter(session_key=chave_antiga).exists())

    def test_logout_marca_sessao_como_inativa(self):
        client = Client()
        self.login(client)
        ...
        self.assertFalse(SessaoUsuario.objects.get(session_key=session_key).ativa)

    def test_middleware_atualiza_ultimo_acesso(self):
        client = Client()
        self.login(client)
        sessao = SessaoUsuario.objects.get(usuario=self.usuario, ativa=True)
        instante_antigo = timezone.now() - timedelta(minutes=10)
        SessaoUsuario.objects.filter(pk=sessao.pk).update(ultimo_acesso=instante_antigo)

        client.get(reverse('dashboard'))

        sessao.refresh_from_db()
        self.assertGreater(sessao.ultimo_acesso, instante_antigo)

    def test_middleware_bloqueia_sessao_antiga_e_exibe_mensagem(self):
        primeiro = Client()
        segundo = Client()
        self.login(primeiro)
        self.login(segundo)

        response = primeiro.get(reverse('dashboard'), follow=True)
        ...
        self.assertTrue(LogAtividade.objects.filter(
            usuario=self.usuario,
            descricao__icontains='outro dispositivo',
        ).exists())

    def test_usuarios_diferentes_podem_permanecer_logados(self):
        primeiro = Client()
        segundo = Client()

        self.login(primeiro)
        self.login(segundo, username='outro-usuario')

        self.assertEqual(SessaoUsuario.objects.filter(ativa=True).count(), 2)
        self.assertTrue(primeiro.get(reverse('dashboard')).wsgi_request.user.is_authenticated)
        self.assertTrue(segundo.get(reverse('dashboard')).wsgi_request.user.is_authenticated)

    def test_novo_login_de_superusuario_e_permitido(self):
        superusuario = get_user_model().objects.create_superuser(
            username='super-sessao',
            password='senha',
            email='super@gestix.test',
        )
        primeiro = Client()
        segundo = Client()
        self.login(primeiro, username=superusuario.username)

        response = self.login(segundo, username=superusuario.username)

        self.assertEqual(response.status_code, 302)
        self.assertEqual(SessaoUsuario.objects.filter(usuario=superusuario, ativa=True).count(), 1)
...
class PermissoesAplicadasModulosTests(TestCase):
    ...
    def test_vendedor_mantem_produtos(self):
        self.client.force_login(self.vendedor)
        ...
        response = self.client.get(reverse('produtos:delete', args=[self.produto.pk]))

        self.assertEqual(response.status_code, 200)

    def test_vendedor_acessa_produtos_com_acoes_de_manutencao(self):
        self.client.force_login(self.vendedor)
        ...
        self.assertContains(response, reverse('produtos:detail', args=[self.produto.pk]))
        self.assertContains(response, reverse('produtos:create'))
        self.assertContains(response, reverse('produtos:update', args=[self.produto.pk]))
        self.assertContains(response, reverse('produtos:delete', args=[self.produto.pk]))

        response = self.client.get(reverse('produtos:detail', args=[self.produto.pk]))

        self.assertEqual(response.status_code, 200)
        self.assertContains(response, self.produto.nome)
        self.assertContains(response, reverse('produtos:update', args=[self.produto.pk]))

    def test_vendedor_acessa_contas_receber_por_url_manual(self):
        self.client.force_login(self.vendedor)

        response = self.client.get(reverse('contas_receber:list'))
...
class ConfiguracaoRedeLocalTests(SimpleTestCase):
    def test_exemplo_de_ambiente_configura_host_e_origem_da_rede(self):
        ...
        self.assertIn('ALLOWED_HOSTS=localhost,127.0.0.1', env_example)
        self.assertIn('hostname real e HOSTNAME.local', env_example)
    ...
    def test_launcher_mantem_localhost_e_informa_endereco_de_rede(self):
        ...
        self.assertIn('discover_server', launcher)
        self.assertIn('discovered_server.json', launcher)

    def test_settings_incluem_hostname_atual_automaticamente(self):
        from django.conf import settings
        from gestix.network_info import required_hosts
