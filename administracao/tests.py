...
from .backup_service import ErroBackup, gerar_backup, restaurar_backup
...
class BackupAdministracaoTests(TestCase):
    ...
    def test_tela_principal_exibe_acoes_e_confirmacao(self):
        self.client.force_login(self.admin)
        resposta = self.client.get(reverse('administracao:backup'))
        self.assertContains(resposta, 'Gerar Backup')
        self.assertContains(resposta, 'Restaurar Backup')
        self.assertContains(resposta, 'Histórico de operações')
        self.assertContains(
            resposta,
            'A restauração substituirá os dados atuais do sistema. Deseja continuar?',
        )

    def test_download_backup_exige_administrador_e_entrega_arquivo_privado(self):
        backup_root = Path(settings.BACKUP_ROOT)
        backup_root.mkdir(parents=True, exist_ok=True)
        nome_arquivo = 'gestix_backup_download.dump'
        caminho = backup_root / nome_arquivo
        ...
        try:
            registro = BackupRegistro.objects.create(
                ...
                usuario=self.admin,
                status=BackupRegistro.Status.SUCESSO,
                mensagem='Backup pronto para download.',
            )

            self.client.force_login(self.gerente)
            resposta_negada = self.client.get(
                reverse('administracao:backup_baixar', args=[registro.pk])
            )
            self.assertRedirects(resposta_negada, reverse('dashboard'))
    ...
    def test_servico_backup_registra_historico_e_auditoria(self):
        banco_postgres = {
            'nome': 'gestix',
            'usuario': 'gestix',
            'senha': 'gestix',
        ...
        with TemporaryDirectory() as tmpdir, override_settings(BACKUP_ROOT=Path(tmpdir)):
            def executar_mock(comando):
                caminho = Path(comando[comando.index('-f') + 1])
                caminho.write_bytes(b'PGDMP')

            with patch('administracao.backup_service._configuracao_banco', return_value=banco_postgres), \
                 patch('administracao.backup_service._executar', side_effect=executar_mock):
                registro = gerar_backup(usuario=self.admin)

        self.assertEqual(registro.status, BackupRegistro.Status.SUCESSO)
        self.assertEqual(registro.usuario, self.admin)
        self.assertRegex(registro.nome_arquivo, r'^gestix_backup_\d{8}_\d{6}\.dump$')
        self.assertTrue(
            LogAtividade.objects.filter(
                acao=LogAtividade.Acao.BACKUP, objeto_id=str(registro.pk)
            ).exists()
        )

    def test_restauracao_invalida_registra_historico_de_erro(self):
        with TemporaryDirectory() as tmpdir, override_settings(BACKUP_ROOT=Path(tmpdir)):
            caminho = Path(tmpdir) / 'invalido.txt'
            caminho.write_bytes(b'invalido')
            with self.assertRaises(ErroBackup):
                restaurar_backup(caminho, usuario=self.admin)

        registro = BackupRegistro.objects.get(tipo=BackupRegistro.Tipo.RESTAURACAO)
        self.assertEqual(registro.status, BackupRegistro.Status.ERRO)
        self.assertEqual(registro.nome_arquivo, 'invalido.txt')
        self.assertTrue(
            LogAtividade.objects.filter(
                acao=LogAtividade.Acao.ERRO, objeto_id=str(registro.pk)
            ).exists()
        )
