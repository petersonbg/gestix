...
class ContaPagarCreateView(ContaPagarPermissaoMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        registrar_log(self.request.user, 'CRIACAO', 'contas_pagar', f'Conta {form.instance.descricao} criada.', objeto=form.instance, request=self.request)
        return response
    ...
    def post(self, request, pk):
        ...
        if form.is_valid():
            try:
                ...
                registrar_log(request.user, 'PAGAMENTO', 'contas_pagar', f'Conta #{conta.pk} paga.', objeto=conta, request=request)
    ...
    def post(self, request, pk):
        ...
        if form.is_valid():
            try:
                ...
                registrar_log(request.user, 'CANCELAMENTO', 'contas_pagar', f'Conta #{conta.pk} cancelada.', objeto=conta, request=request)
                return redirect(conta.get_absolute_url())
        return render(request, self.template_name, {'conta': conta, 'form': form})
