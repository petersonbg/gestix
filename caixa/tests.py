...
class CaixaTests(TestCase):
    ...
    def test_fechamento_pela_view_registra_auditoria(self):
        caixa = Caixa.abrir(usuario=self.user, valor_inicial=Decimal('100.00'))
        self.client.force_login(self.user)
        resposta = self.client.post(
            reverse('caixa:fechar'),
            {
                'valor_fechamento_informado': '100.00',
                'observacao_fechamento': 'Fechamento auditado',
            },
        )
        self.assertRedirects(resposta, reverse('caixa:historico'))
