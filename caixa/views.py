    ...
    def post(self, request):
        ...
        if form.is_valid():
            try:
                ...
                registrar_log(request.user, 'ABERTURA_CAIXA', 'caixa', f'Caixa #{caixa.pk} aberto.', objeto=caixa, request=request)
                return redirect('caixa:atual')
    ...
    def post(self, request):
        ...
        if form.is_valid():
            try:
                ...
                registrar_log(request.user, 'PAGAMENTO', 'caixa', f'{self.config["titulo"]}: movimentação #{movimentacao.pk} lançada.', objeto=movimentacao, request=request)
                return redirect('caixa:atual')
        return render(request, self.template_name, {'form': form, 'caixa': caixa, 'titulo': self.config['titulo']})
    ...
    def post(self, request):
        ...
        if form.is_valid():
            try:
                ...
                registrar_log(request.user, 'FECHAMENTO_CAIXA', 'caixa', f'Caixa #{caixa.pk} fechado.', objeto=caixa, request=request)
